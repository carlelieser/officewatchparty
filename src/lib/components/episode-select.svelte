<script lang="ts">
	...
	let {episodes, selected = $bindable<Episode | null>(null), onchange, class: className}: EpisodeSelectProps = $props();
...
<Popover.Root bind:open>
    <Popover.Trigger>
        {#snippet child({props})}
            <Button {...props} variant="outline" class={cn("max-w-md min-w-0 flex flex-row overflow-hidden", className)}>
                <TvIcon/>
